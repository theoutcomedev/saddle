...
文件工具的路径摘要（`read`／`write`／`edit` 参数中的 `path` 或 `file_path`）渲染为静止状态下即带下划线的链接，并使用 pointer 光标。点击路径会把该路径显示在 Workbench 文件面板中，相对路径以会话 cwd 为基准解析（[路由决策](../simplification/2026-09-13-chat-file-links-route-to-the-pane.zh.md)）；下方的桌面交接由该面板自身的「打开」控件到达。带文件链接的行关闭参数展开（左侧图标不可点）；工具行（含 bash 与 todo 注册）去掉整行点击、整行悬停底色，以及点击打开 details 的手势。details 面板及其 inject 面仍保留供程序化选择；工具行不再驱动它们。
...
点击工具行中的文件路径会在 Workbench 文件面板中显示该路径；用桌面默认应用打开它是该面板自身的控件。非文件工具行只是不可交互的摘要（行内已有的展开开关仍保留）。远程或非回环客户端无法调用 `host.openPath`。

## 风险

- 没有 `xdg-open` 的桌面 Linux 宿主，以及 Windows 互操作（`wslpath` 加 `powershell.exe`）不可用的 WSL 宿主，会使 RPC 失败；文件面板的「打开」控件不报告任何信息，读者看不到失败原因。
