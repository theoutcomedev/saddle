...
1. **The preview card is token-driven.** `--dsw-specific-hovercard-bg`, `-label`, `-label-secondary`, and `-label-tertiary` replaced the component's literal colours, so the card is drawn from tokens rather than from `ui-primitives`/`ui-workspace` CSS. Both built-in palettes declare them in `design-platform.css`; [the light-palette preview-card note](../bug-fix/2026-09-14-light-palette-hover-card-surface.md) owns which values each one names. `ui-primitives/HoverCard.module.css` and `ui-workspace/Rows.module.css` consume the tokens through the CSS-module classes they already owned, and neither reads a literal colour any more.
2. **The creative palettes override them.** The `HOVER_CARD_TOKENS` group in `ui-theme/src/client/index.ts` points the four names at the palette's own `--dsw-alias-bg-layer-3` and label aliases. It joins `CHAT_CONTENT_TOKENS` in one `CREATIVE_TOKENS` group spread into the six palettes, so a seventh palette inherits the derivation by spreading that group.
...
- **Reusing an existing alias directly (`--dsw-alias-bg-layer-3`) in the component CSS.** Rejected: in the component it would repaint the card in `light` and `dark` together. The light half of that alternative was later taken up in the sheet, scoped so `dark` keeps the figma ramp — [the light-palette preview-card note](../bug-fix/2026-09-14-light-palette-hover-card-surface.md).
...
- `dark` keeps the figma card byte-for-byte; `light` and the six creative palettes draw it from their own elevations, and their label ramp follows the palette's contrast rather than a fixed white/grey set.
...
- `packages/client/ui-theme/src/styles/design-platform.css`: the four `--dsw-specific-hovercard-*` tokens of both built-in palettes.
