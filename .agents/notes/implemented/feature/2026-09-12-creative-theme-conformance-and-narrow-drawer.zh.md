...
1. **预览卡片改为由 token 驱动。** `design-platform.css` 从既有静态中性色（bluish-850／00／300／400，即原有的确切取值）一次性声明 `--dsw-specific-hovercard-bg`、`-label`、`-label-secondary` 与 `-label-tertiary`，并且刻意不按配色方案重复声明，因此 `light` 与 `dark` 的渲染结果与改动前完全一致。`ui-primitives/HoverCard.module.css` 与 `ui-workspace/Rows.module.css` 通过原本就属于它们的 CSS Module 类消费这些 token，两者都不再读取字面颜色。
2. **只有创意调色板覆盖这些 token。** `ui-theme/src/client/index.ts` 中的 `HOVER_CARD_TOKENS` 组把这四个名字指向调色板自身的 `--dsw-alias-bg-layer-3` 与标签别名。它与 `CHAT_CONTENT_TOKENS` 一起并入 `CREATIVE_TOKENS` 组，由六个调色板共同展开，因此第七个调色板只要展开该组即可继承这套派生。
...
- **在组件 CSS 中直接复用既有的别名（`--dsw-alias-bg-layer-3`）。** 否决：这会让 `light` 与 `dark` 下的卡片也被重绘，而 figma 卡片刻意不这样做。
...
- `light` 与 `dark` 保持 figma 卡片原样不变；六个创意调色板则用自身的层级重绘卡片，其标签色阶跟随调色板的对比度，而不再使用固定的白／灰取值。
...
- `packages/client/ui-theme/src/styles/design-platform.css`：四个 `--dsw-specific-hovercard-*` token。
