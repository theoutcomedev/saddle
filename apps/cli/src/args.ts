...
const HELP_EXAMPLES = `
Examples:
  saddle --profile web                          boot the web profile (same as: saddle web)
  saddle --profile headless "run the tests"     answer one task, print the result, and exit
  saddle --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  saddle --profile tui --resume <session>       arguments after the launcher flags reach the app
  saddle --profile web --help                   the web app's own flags and help
  saddle plugin --profile tui add <package>     install a plugin into the tui profile
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    .description('saddle: boot a Saddle OS profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `saddle -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: saddle --profile <name> --help)')
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: saddle web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
