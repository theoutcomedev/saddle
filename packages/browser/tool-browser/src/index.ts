...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.systemPrompt.section({
    ...
    order: 110,
    text: [
      'CRITICAL: Browser tools are ONLY for interactive automation requiring a real browser session (e.g. clicking buttons, filling multi-step forms, authenticated logins, testing dynamic client-rendered web apps, capturing screenshots/PDFs, or when the user explicitly requests using a browser).',
      'DO NOT use browser tools for general web lookups, reading public web pages, checking documentation, or retrieving facts when faster tools (such as web search, curl, or HTTP fetch) can achieve the goal. Browser automation has high latency and resource overhead; prefer direct reading tools first.',
      'When interactive browser automation is genuinely needed: call browser_navigate first to load the page (you can pass profile: "name" to resume cookies/logins).',
      'Use browser_get_content to inspect forms, buttons, and recommended CSS selectors.',
      'Use browser_click with selector (e.g. "#submit-btn"), button text, or coordinates.',
      'Use browser_type to fill inputs (with optional pressEnter: true).',
      'Use browser_screenshot to capture visual state, browser_pdf to export styled documents, and browser_download_file to save authenticated downloads.',
      'Always call browser_session_end when finished browsing to free container resources.',
    ].join(' '),
  })
