...
function scriptedApi(overrides: {
  ...
  connections?: Partial<ApiProxy['connections']>
  ...
  return {
    sessions: {
      ...
      rewind: r => ok(r, { sessionId: sid('s-rewind') }),
      rewindCollisions: r => ok(r, { collisions: [] }),
    ...
    schedules: {
      list: r => ok(r, { tasks: [] }),
      create: r => ok(r, { task: { id: 'task-1', name: 'Task', prompt: 'Do work', cadenceType: 'interval', cadenceValue: '30', cadenceLabel: 'Every 30m', enabled: true, targetMode: 'new-session', createdAt: new Date().toISOString() } }),
      update: r => ok(r, { task: { id: 'task-1', name: 'Task', prompt: 'Do work', cadenceType: 'interval', cadenceValue: '30', cadenceLabel: 'Every 30m', enabled: true, targetMode: 'new-session', createdAt: new Date().toISOString() } }),
      delete: r => ok(r, { success: true }),
      trigger: r => ok(r, { runId: 'run-1', status: 'started' }),
      logs: r => ok(r, { runs: [] }),
    },
