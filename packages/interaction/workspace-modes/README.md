...
The projection folds the last request into `{ mode, at }`, where `at` is the recording event's own sequence. The sequence is the whole reason the key exists: two requests for the same mode are two instructions, and a renderer applying by value alone would ignore the second one — so "put me back in zen mode" works after the person has left zen, while the exit is never undone by the instruction that entered it.

## Model Experience

**Tool.** `set_workspace_mode` takes one required enum parameter over the catalogue (`standard`, `focus`, `zen`) and returns one line naming the mode and what it means. The description lists every mode with its purpose, derived from `src/catalogue.ts` so the enum and the prose cannot drift apart.

**Logged state.** One `workspace/mode` event per call, log-only and non-surface: it is not part of the transcript the model reads back, and the model's view of what happened is the tool result alone.
