/**
 * The `workspaceMode` projection provider: the browser half reads the shape a
 ...
 * renderer tells two requests for the same mode apart), starts as null, is
...
function request<P>(payload: P): RpcRequest<P> {
  return { rpcId: RpcId(`workspace-mode-proj-${String(nextRpc++)}`), payload }
}
...
async function harness(withModes: boolean): Promise<Bench> {
  ...
  if (withModes) await ctx.plugin(WorkspaceModes)
...
describe('the workspaceMode projection provider', () => {
  it('serves null before the session asks for a shape', async () => {
    ...
    expect(projections?.values.workspaceMode).toBeNull()
  ...
  it('serves the last request with the sequence that recorded it', async () => {
    const bench = await harness(true)
    seedMessage(bench.session)
    bench.session.append('workspace/mode', { mode: 'focus' })
    bench.session.append('workspace/mode', { mode: 'zen' })
    const projections = await bench.tailProjections()
    expect(projections?.values.workspaceMode).toEqual({ mode: 'zen', at: bench.session.seq - 1 })
  })

  it('tells two requests for the same mode apart', async () => {
    // The chip consumes an instruction by its sequence: without that, asking
    // for a mode the person has since left would be indistinguishable from the
    ...
    const firstState = first?.values.workspaceMode as { at: number } | undefined
    const secondState = second?.values.workspaceMode as { at: number } | undefined
  ...
  it('has no key in a composition without this package', async () => {
    ...
    seedMessage(bench.session)
    const projections = await bench.tailProjections()
    expect('workspaceMode' in (projections?.values ?? {})).toBe(false)
  })
  ...
  it('drops the key when the fiber unloads', async () => {
    ...
    expect((await bench.tailProjections())?.values.workspaceMode).toBeNull()
    await fiber.dispose()
    expect('workspaceMode' in ((await bench.tailProjections())?.values ?? {})).toBe(false)
  })
})
